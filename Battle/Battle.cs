                ...
                else
                {
                    ...
                    if (W20 <= MonsterAttack)
                    {
                        ...
                        else
                        {
                            ...
                            Console.ForegroundColor = ConsoleColor.Red;
                            HeroArmorValue = hero.CalculateArmorValue();
...
*********************************", hero.AnzLittleHPPotions, hero.AnzMiddleHPPotions, hero.AnzBigHPPotions);
        ...
        {
            bool toMuchInInv = true;

            var count = hero.Inventar.Count(s => s.Contains(cItem));
        ...
        void RandomDrop(int MonsterEXP, Hero hero)
        {
            ...
            if (isInInv == false)
            {
                if (OutputItem[2] == "weapon")
                {
                    string weapon = OutputItem[0] + ";" + OutputItem[1] + ";" + OutputItem[3] + ";" + OutputItem[4];
                    hero.WeaponInventar.Add(weapon);
                }
                if (OutputItem[2] == "item")
                {
                    hero.Inventar.Add(ChoosenItem);
                }
            }
            else
            {
